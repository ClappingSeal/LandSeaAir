...
from locking_env_TD3_pos_straight_learning_plot import LockingEnv
...
# 환경 및 모델 로드
env = LockingEnv()
model = TD3.load("tracking_model_td3_pos_curve_1004")
...
for test in range(num_tests):
    ...
    while not done:
        ...
        # 직사각형 그리기
        rect_bottomleft_x = circle_center[0] - 16
        rect_bottomleft_y = circle_center[1] - 12
        rectangle = patches.Rectangle((rect_bottomleft_x, rect_bottomleft_y), 32, 24, fill=False, color=(1, 0, 0, 0.5), linewidth=rectangle_linewidth, label="camera")
